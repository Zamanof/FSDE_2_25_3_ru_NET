...
2. Heap
    - Managed Heap -Управляемый куча, где размещаются объекты, созданные с помощью оператора new. 
      ...
      Managed Heap состоит из нескольких поколений (Generations), которые помогают 
      оптимизировать процесс сборки мусора.   
                * Generation 0 - 
                ...
                * Generation 2 - Размер Genereation 2 примерно более 10 МБ
...
//GC.Collect();
//Console.WriteLine("After first cleaning:");
//Console.WriteLine($"Generation: {GC.GetGeneration(garbageHelper)}");
//GC.Collect();
//Console.WriteLine("After second cleaning:");
//Console.WriteLine($"Generation: {GC.GetGeneration(garbageHelper)}");

//Console.WriteLine($"Max Genereation: {GC.MaxGeneration}");
