...
{
    if(i % 2 == 0) teacher.OnExam += students[i].Exam;
    else teacher2.OnExam += students[i].Exam;
}

teacher.StartExam(new ExamEventArgs(){Task = "Solve C++ Tik Tak Toe"});
teacher2.StartExam(new ExamEventArgs(){Task = "Solve C# Battleship"});